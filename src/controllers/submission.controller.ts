...
export const resubmitSubmissionController = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const studentId = req.user.id; // always comes from token
    const { answers } = req.body;

    const result = await SubmissionService.resubmitSubmission(
      studentId,
      submissionId,
      answers
    );
